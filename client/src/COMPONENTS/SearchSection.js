...
const SearchSection = ({ books }) => {
  ...
  for (let i = 0; i < books.length; i++) {
    if (books[i].author.toLowerCase().includes(search.toLowerCase())) {
      // console.log(books[i].author);
      results[i] = books[i];
      authorCount += 1;
      author = true;
    } else if (books[i].title.toLowerCase().includes(search.toLowerCase())) {
      results[i] = books[i];
      titleCount += 1;
      title = true;
    } else if (books[i].year.toString() === search.toLowerCase()) {
          ...
          results.length ? (
            <>
              ...
              <div className="countSection">
                {year ? (
                  <pre style={{ fontSize: "34px", color: "var(--linkText)" }}>
                    No. of Books published in the year "{search}" :- {yearCount}
                  </pre>
                ) : title ? (
                  <pre style={{ fontSize: "34px", color: "var(--linkText)" }}>
                    No. of Books published in this title "{search}" :-{" "}
                    {titleCount}
                  </pre>
                ) : author ? (
                  <pre style={{ fontSize: "34px", color: "var(--linkText)" }}>
                    No. of Books published by this author "{search}" :-{" "}
                    {authorCount}
                  </pre>
                ) : (
                  <></>
